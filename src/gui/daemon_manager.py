...
from gui_tools import CommandLineArgs, default_session_root, ErrDaemon, _translate
...
class DaemonManager(QObject):
    ...
    def callDaemon(self):
        ...
        if not server:
            sys.stderr.write('GUI can not call daemon, GUI OSC server is missing.\n')
            return
    ...
    def announceTimerOut(self):
        if self.launched_before:
            self._signaler.daemon_url_request.emit(ErrDaemon.NO_ANNOUNCE, self.url)
        else:
            sys.stderr.write(_translate('error', "No announce from ray-daemon. RaySession can't works. Sorry.\n"))
            app.quit()
            
    def receiveAnnounce(self, src_addr, version, server_status, options, session_root, is_net_free):
        self.announce_timer.stop()
        
        if version.split('.')[:1] != ray.VERSION.split('.')[:1]:
            #works only if the two firsts digits are the same (ex: 0.6)
            self._signaler.daemon_url_request.emit(ErrDaemon.WRONG_VERSION, self.url)
            self.disannounce(src_addr)
            return
        
        if CommandLineArgs.net_session_root and session_root != CommandLineArgs.net_session_root:
            self._signaler.daemon_url_request.emit(ErrDaemon.WRONG_ROOT, self.url)
            self.disannounce(src_addr)
            return
        
        if not is_net_free:
            self._signaler.daemon_url_request.emit(ErrDaemon.FORBIDDEN_ROOT, self.url)
            self.disannounce(src_addr)
            return
    ...
    def start(self):
        ...
        server = GUIServerThread.instance()
        if not server:
            sys.stderr.write("impossible for GUI to launch daemon. server missing.\n")
