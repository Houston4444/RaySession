...
class MainWindow(QMainWindow):
    def __init__(self, session):
        ...
        self.ui.actionSaveAllFromSavedClient.toggled.connect(self.saveAllFromClientToggled)
        self.ui.actionBookmarkSessionFolder.toggled.connect(self.bookmarkSessionFolderToggled)
        self.ui.actionDesktopsMemory.toggled.connect(self.desktopsMemoryToggled)
        ...
        if self.ui.actionControlMenu.icon().isNull():
            self.ui.actionControlMenu.setIcon(QIcon.fromTheme('configuration_section'))
        ...
        self.ui.actionDuplicateSession.setIcon(RayIcon('xml-node-duplicate', dark))
        self.ui.actionSaveTemplateSession.setIcon(RayIcon('document-save-as-template', dark))
    ...
    def setNsmLocked(self, nsm_locked):
        ...
        self.ui.stackedWidgetSessionName.setEditable(nsm_locked and not CommandLineArgs.out_daemon)
        self.ui.actionRenameSession.setEnabled(nsm_locked and not CommandLineArgs.out_daemon)
    
    def setDaemonOptions(self, options):
        self.ui.actionSaveAllFromSavedClient.setChecked(bool(options & ray.Option.SAVE_FROM_CLIENT))
        self.ui.actionBookmarkSessionFolder.setChecked( bool(options & ray.Option.BOOKMARK_SESSION))
        self.ui.actionDesktopsMemory.setChecked(bool(options & ray.Option.DESKTOPS_MEMORY))
        
        has_wmctrl = bool(options & ray.Option.HAS_WMCTRL)
        self.ui.actionDesktopsMemory.setEnabled(has_wmctrl)
        if has_wmctrl:
            self.ui.actionDesktopsMemory.setText(_translate('actions', 'Desktops Memory'))
    ...
    def quitApp(self):
        if self._daemon_manager.launched_before and not CommandLineArgs.under_nsm:
            self.quitAppNow()
            return
    ...
    def abortCopy(self):
        ...
        if not self._session.server_status in (ray.ServerStatus.PRECOPY, ray.ServerStatus.COPY):
            return
    ...
    def abortCopyClient(self, client_id):
        ...
        client = self._session.getClient(client_id)
        if not client or not client.status in (ray.ClientStatus.COPY, ray.ClientStatus.PRECOPY):
            return
    ...
    def showDaemonUrlWindow(self, err_code, ex_url=''):
        ...
        dialog.exec()
        if not dialog.result():
            if CommandLineArgs.under_nsm and self._daemon_manager.launched_before:
                app.quit()
            return
    ...
    def serverSessionRenameable(self, renameable):
        ...
        self.ui.stackedWidgetSessionName.setEditable(renameable and self._session.server_status == ray.ServerStatus.READY and not CommandLineArgs.out_daemon)
    ...
    def serverProgress(self, progress):
        self.ui.lineEditray.ServerStatus.setProgress(progress)
    ...
    def serverChangeServerStatus(self, server_status):
        ...
        self.ui.frameCurrentSession.setEnabled(bool(server_status != ray.ServerStatus.OFF))
        ...
        close_or_off = bool(server_status in (ray.ServerStatus.CLOSE, ray.ServerStatus.OFF))
        ...
        self.ui.actionOpenSessionFolder.setEnabled(bool(server_status != ray.ServerStatus.OFF))
        
        self.ui.stackedWidgetSessionName.setEditable(ready and self._session.is_renameable)
        if CommandLineArgs.under_nsm and not CommandLineArgs.out_daemon and ready and self._session.is_renameable:
