...
class OpenSessionDialog(ChildDialog):
    ...
    def serverStatusChanged(self, server_status):
        self.ui.toolButtonFolder.setEnabled(bool(server_status == ray.ServerStatus.OFF))
        
        self.server_will_accept = bool(server_status in (ray.ServerStatus.OFF, ray.ServerStatus.READY) and not self.server_copying)
        self.preventOk()
...
class NewSessionDialog(ChildDialog):
    def __init__(self, parent, duplicate_window = False):
        ...
        if self.is_duplicate:
            ...
            self.ui.labelNewSessionName.setText(_translate('Duplicate', 'Duplicated session name :'))
    ...
    def serverStatusChanged(self, server_status):
        ...
        self.server_will_accept = bool(server_status in (ray.ServerStatus.OFF, ray.ServerStatus.READY) and not self.server_copying)
        if self.is_duplicate:
            self.server_will_accept = bool(server_status == ray.ServerStatus.READY and not self.server_copying)
...
class SaveTemplateClientDialog(AbstractSaveTemplateDialog):
    def __init__(self, parent):
        ...
        self._signaler.user_client_template_found.connect(self.addTemplatesToList)
    ...
    def serverStatusChanged(self, server_status):
        self.server_will_accept = bool(not server_status in (ray.ServerStatus.OFF, ray.ServerStatus.CLOSE) and not self.server_copying)
...
class ClientTrashDialog(ChildDialog):
    ...
    def removeClient(self):
        self.toDaemon('/ray/trash/remove_definitely', self.client_data.client_id)
        self.reject()
...
class AbortSessionDialog(ChildDialog):
    ...
    def serverStatusChanged(self, server_status):
        self.ui.pushButtonAbort.setEnabled(not bool(server_status in (ray.ServerStatus.CLOSE, ray.ServerStatus.OFF, ray.ServerStatus.COPY)))
        if server_status == ray.ServerStatus.OFF:
            self.reject()
...
class AbortServerCopyDialog(ChildDialog):
    ...
    def serverStatusChanged(self, server_status):
        if not server_status in (ray.ServerStatus.PRECOPY, ray.ServerStatus.COPY):
            self.reject()
...
class QuitAppDialog(ChildDialog):
    def __init__(self, parent):
        ...
        self.ui.labelExecutable.setText(original_text % ('<strong>%s</strong>' % self._session.name))
    ...
    def serverStatusChanged(self, server_status):
        ...
        self.ui.pushButtonSaveQuit.setEnabled(bool(server_status == ray.ServerStatus.READY))
        self.ui.pushButtonQuitNoSave.setEnabled(bool(server_status != ray.ServerStatus.CLOSE))
...
class AddApplicationDialog(ChildDialog):
    def __init__(self, parent):
        ...
        self._signaler.user_client_template_found.connect(self.addUserTemplates)
        self._signaler.factory_client_template_found.connect(self.addFactoryTemplates)
        self.toDaemon('/ray/server/list_user_client_templates')
...
class StopClientDialog(ChildDialog):
    def __init__(self, parent, client_id):
        ...
        self._signaler.client_status_changed.connect(self.serverUpdatesClientStatus)
...
class DaemonUrlWindow(ChildDialog):
    def __init__(self, parent, err_code, ex_url):
        ...
        if err_code == ErrDaemon.NO_ANNOUNCE:
            ...
            error_text = _translate("url_window", "<p>daemon at<br><strong>%s</strong><br>has a loaded self._session.<br>It can't be used for slave session</p>") % ex_url
            ...
            error_text = _translate("url_window", "<p>daemon at<br><strong>%s</strong><br>uses a forbidden session root folder !<.p>") % ex_url
