...
class PatchRemote(BunServerThread):
    ...
    def _connection_added(self, osp: OscPack):
        ...
        if self.startup_received:
            self.ev.add_event(
                PatchEvent.CONNECTION_ADDED, jport_out.name, jport_in.name)
    ...
    def _port_added(self, osp: OscPack):
        ...
        if self.startup_received:
            self.ev.add_event(
                PatchEvent.PORT_ADDED, port_data.name, mode, port_data.type)
    ...
    def _port_removed(self, osp: OscPack):
        ...
        self.ev.add_event(PatchEvent.PORT_REMOVED, jack_port.name,
