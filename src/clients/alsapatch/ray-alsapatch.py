...
# ray-alsapatch is an executable launchable in NSM (or Ray) session.
# It restores ALSA connections.
# To avoid many problems, the connection processus is slow.
# Connections are made one by one, as ray-jackpatch does.
...
# ray-alsapatch client once the session is already loaded.
...
jack_ports = dict[PortMode, list[AlsaPort]]()
for port_mode in PortMode:
    jack_ports[port_mode] = list[AlsaPort]()
...
def port_added(port_name: str, port_mode: int):
    port = AlsaPort()
    port.name = port_name
...
def port_removed(port_name: str, port_mode: PortMode):
        ...
        if to_disc_connections:
            for to_disc_con in to_disc_connections:
                if to_disc_con in connection_list:
                    alsa_mng.connect_ports(*to_disc_con, disconnect=True)
