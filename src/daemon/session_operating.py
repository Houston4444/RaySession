...
for better organization.""")
    ...
    def save_client_as_template(self, client: Client, template_name: str):
        ...
        if client_files:
            ...
            self.file_copier.start_client_copy(
                ...
                self.save_client_as_template_aborted,
                [client, template_name])
        else:
            self.save_client_as_template_step_1(client, template_name)

    def save_client_as_template_step_1(
            self, client: Client, template_name: str):
        client.set_status(client.status) # see set_status to see why

        if client.prefix_mode is not ray.PrefixMode.CUSTOM:
            client.adjust_files_after_copy(
                template_name, ray.Template.CLIENT_SAVE)
        ...
        self._send_reply('client template created')
        
    def save_client_as_template_aborted(
            self, client: Client, template_name: str):
