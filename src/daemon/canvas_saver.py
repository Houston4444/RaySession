...
class CanvasSaver(ServerSender):
    ...
    def send_session_group_positions(self):
        ...
        for gp_name, ptov in self.pretty_names_session.groups.items():
            ms_gui.add(RGP.UPDATE_GROUP_PRETTY_NAME,
                       gp_name, ptov.pretty)
            ms_gui.add(RP.GROUP_PRETTY_NAME,
                       gp_name, ptov.pretty, ptov.above_pretty)
        
        ms_patch.add(RP.GROUP_PRETTY_NAME, '', '', '')

        for port_name, ptov in self.pretty_names_session.ports.items():
            ms_gui.add(RGP.UPDATE_PORT_PRETTY_NAME,
                       port_name, ptov.pretty)
            ms_gui.add(RP.PORT_PRETTY_NAME,
                       port_name, ptov.pretty, ptov.above_pretty)

        ms_patch.add(RP.PORT_PRETTY_NAME, '', '', '')
        ms_gui.add(RGP.VIEWS_CHANGED, mixed_views_str)
    ...
    def send_all_group_positions(self, gui: 'Gui'):
        ...
        for view_index in self.views_config.keys():
            for gpos in self.views_config.iter_group_poses(
                    view_num=view_index):
                ms.add(RGP.UPDATE_GROUP_POSITION,
        ...
        for pt_name, pretty_port in self.pretty_names_config.ports.items():
            ms.add(RGP.UPDATE_PORT_PRETTY_NAME,
